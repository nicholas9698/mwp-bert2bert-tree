...
from src.models import BertEncoderDecoderModelFroTree, BertTokenizer
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    return loss.item(), seq2seq_loss.item(), total_loss.item()


def evaluate_tree(input_batch, input_length, generate_nums, model: BertEncoderDecoderModelFroTree, predict, generate, merge, output_lang, num_pos, attribute_pos, beam_size=5, max_length=MAX_OUTPUT_LENGTH):
    ...
    encoder_outputs, problem_output = model.evaluate(input_batch, num_pos, attribute_pos)
    # encoder_outputs, problem_output = model.evaluate(input_batch, None, None)
