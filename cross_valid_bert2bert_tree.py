...
for fold in range(5):
    ...
    model = BertEncoderDecoderModelFroTree.from_encoder_decoder_pretrained('/root/autodl-tmp/MWP-BERT', '/root/autodl-tmp/MWP-BERT', tie_encoder_decoder=True)
    ...
    for epoch in range(n_epochs):
        ...
        print("fold {0} epoch {1}:".format(fold+1, epoch+1))
        start = time.time()
...
equ_ac, val_ac, total_count = 0, 0, 0
for bl in range(len(best_acc_fold)):
    equ_ac += best_acc_fold[bl][0]
    val_ac += best_acc_fold[bl][1]
    total_count += best_acc_fold[bl][2]
    print(best_acc_fold[bl])
print(equ_ac / float(total_count), val_ac / float(total_count))